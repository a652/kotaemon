...
class CohereReranking(BaseReranking):
    ...
    def run(self, documents: list[Document], query: str) -> list[Document]:
        ...
        if not self.cohere_api_key or "COHERE_API_KEY" in self.cohere_api_key:
            ...
            return documents
        base_url = config('CO_API_URL') 
        print(f"cohere reranking api key: {self.cohere_api_key}, base_url: {base_url}, model: {self.model_name}")

        cohere_client = cohere.Client(self.cohere_api_key, base_url=base_url)
