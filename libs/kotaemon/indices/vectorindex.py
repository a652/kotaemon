...
class VectorIndexing(BaseIndexing):
    ...
    def split_documents(self, documents, max_length=100):
        """
        把一个列表拆分成多个列表，每个列表的最大长度为max_length。
        
        :param documents: list[Document] 要拆分的列表
        :param max_length: int 每个拆分后的子列表的最大长度
        :return: list[list[Document]] 拆分后的列表
        """
        # 使用列表推导式将列表按max_length长度拆分
        return [documents[i:i + max_length] for i in range(0, len(documents), max_length)]
    ...
    def add_to_vectorstore(self, docs: list[Document]):
        ...
        if self.vector_store:
            ...
            if len(docs) > 100:
                docs_list = self.split_documents(docs)
                for new_docs in docs_list:
                    embeddings = self.embedding(new_docs)
                    print("Adding embeddings to vector store")
                    self.vector_store.add(
                        embeddings=embeddings,
                        ids=[t.doc_id for t in new_docs],
                    )
            else:
